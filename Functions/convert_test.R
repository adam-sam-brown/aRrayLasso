convert.test <- function(conversion.model, src, tgt) {
  ...
  pearson <- rep(NA,M)
  for (m in 1:M) {
    #Taking a log of negative or zero values will throw an error
    #Therefore, we find the intersection of pred and tgt that don't produce NA
    
    compat <- complete.cases(log(predicted[m,])) & complete.cases(log(tgt[m,]))
    
    print(length(which(compat == TRUE)))
